...
func (face Face) Consumer() {
	go kafkaCons.Consumer(face.Brokers, face.Topic, face.GroupId, func(consumer *cluster.Consumer, signals chan os.Signal) {
		for {
			select {
			case part, ok := <-consumer.Partitions():
				...
				go func(pc cluster.PartitionConsumer) {
					for msg := range pc.Messages() {
						...
						out := backend.BroadcastByRedis(ctx, "", config.SocEventFace, value, appid, "", tenantId)
			...
			case <-signals:
				...
				//
				//		value := fmt.Sprintf("%s", msg.Value)
				//		out := backend.BroadcastByRedis(ctx,"", config.SocEventFace, value)
				//
				//		var errStr string
				//		select {
				//		case <-ctx.Done():
