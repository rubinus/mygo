...
func (forman Forman) Consumer() {
	go kafkaCons.Consumer(forman.Brokers, forman.Topic, forman.GroupId, func(consumer *cluster.Consumer, signals chan os.Signal) {
		for {
			select {
			case part, ok := <-consumer.Partitions():
				...
				go func(pc cluster.PartitionConsumer) {
					for msg := range pc.Messages() {
						...
						//fmt.Printf("=receive im_queue=%d %s\n", msg.Offset, msg.Value)
						var errStr string
						var appid string
						var tenantId string
						...
						out := backend.BroadcastByRedis(ctx, "", config.SocEventForman, value, appid, "", tenantId)
			...
			case <-signals:
				fmt.Println("forman no signals ...")

				return
				//case msg, ok := <-consumer.Messages():
				//	if ok {
				//		fmt.Printf("===接收到Kafka forman==%d %s\n", msg.Offset, msg.Value)
				//		ctx, cancel := context.WithTimeout(context.Background(), config.GoroutineTimeout)
				//		defer cancel()
				//
				//		value := fmt.Sprintf("%s", msg.Value)
				//		out := backend.BroadcastByRedis(ctx, "", config.SocEventForman, value)
				//
				//		var errStr string
				//		select {
				//		case <-ctx.Done():
